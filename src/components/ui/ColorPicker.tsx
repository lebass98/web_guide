...
const hexToRgb = (hex: string) => {
    ...
    const r = parseInt(
        hashless.length === 3 ? hashless.slice(0, 1).repeat(2) : hashless.substring(0, 2),
        16
    );
    const g = parseInt(
        hashless.length === 3 ? hashless.slice(1, 2).repeat(2) : hashless.substring(2, 4),
        16
    );
    const b = parseInt(
        hashless.length === 3 ? hashless.slice(2, 3).repeat(2) : hashless.substring(4, 6),
        16
    );
    return { r, g, b };
};
