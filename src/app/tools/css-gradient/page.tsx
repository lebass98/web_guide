...
const hexToRgba = (hex: string, opacity: number = 100) => {
  ...
  const r = parseInt(hashless.length === 3 ? hashless.slice(0, 1).repeat(2) : hashless.slice(0, 2), 16);
  const g = parseInt(hashless.length === 3 ? hashless.slice(1, 2).repeat(2) : hashless.slice(2, 4), 16);
  const b = parseInt(hashless.length === 3 ? hashless.slice(2, 3).repeat(2) : hashless.slice(4, 6), 16);
      ...
      <div className="bg-white rounded-xl shadow-sm border border-zinc-200 overflow-hidden">
          ...
          <div className="flex items-center gap-6 pt-4 lg:pt-0">
            <div className="flex bg-white rounded-md border border-zinc-200 shadow-sm p-1">
              <button
                ...
                className={cn("px-4 py-1.5 text-sm font-semibold rounded transition-colors", gradientType === "linear" ? "bg-zinc-100 text-zinc-900" : "text-zinc-500 hover:text-zinc-700")}
              >
                Linear
              </button>
              <button
                onClick={() => setGradientType("radial")}
                className={cn("px-4 py-1.5 text-sm font-semibold rounded transition-colors", gradientType === "radial" ? "bg-zinc-100 text-zinc-900" : "text-zinc-500 hover:text-zinc-700")}
              >
                Radial
              </button>
            </div>
            ...
            <div className="flex gap-2 border-l border-zinc-200 pl-6 h-8 items-center">
              {presets.map((p, i) => (
                <button
                  ...
                  style={{ background: `linear-gradient(135deg, ${p.join(', ')})` }}
                  title="Preset"
                />
              ))}
            </div>
        ...
        <div className="flex flex-col lg:flex-row p-6 gap-8">
          ...
          <div className="flex flex-col gap-3 flex-1 lg:border-l border-zinc-200 lg:pl-8">
            <label className="text-[10px] font-extrabold text-zinc-400 uppercase tracking-wider">Stops</label>
              ...
              {stops.map(stop => (
                <div 
                  ...
                  className={cn(
                    ...
                    activeStopId === stop.id ? "bg-zinc-100 border-zinc-200 shadow-sm" : "bg-transparent border-transparent hover:bg-zinc-50"
                  )}
                >
                   {/* Color Preview */}
                   <div 
      ...
      <div className="bg-[#2D333B] text-zinc-300 rounded-xl overflow-hidden shadow-lg border border-zinc-800 flex flex-col">
        <div className="flex justify-between items-center p-4 border-b border-zinc-700/50 bg-[#22272E]">
          ...
          <label className="flex items-center gap-2 text-xs text-zinc-400 cursor-pointer hover:text-white transition-colors">
            <input type="checkbox" className="rounded bg-zinc-800 border-zinc-600 text-fuchsia-500 focus:ring-fuchsia-500 focus:ring-offset-zinc-900" /> 
        ...
        <div className="p-6 font-mono text-sm leading-relaxed whitespace-pre-wrap overflow-x-auto selection:bg-[#539bf5] selection:text-white">
          <div><span className="text-[#8ddb8c]">background:</span> {stops[0]?.color};</div>
          <div><span className="text-[#8ddb8c]">background:</span> {cssCode.replace('background: ', '').replace(';', '')};</div>
        </div>
        <button
          onClick={handleCopy}
